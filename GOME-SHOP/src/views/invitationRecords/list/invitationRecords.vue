		...
		<div :class="inWechat ? 'container-inwechat' : 'container-notwechat'">
			...
			<div class="records-num-box" v-if="recordList.length>0">
				<div class="records-num">
					<span>成功邀请 ({{totalRecords}})</span>
				</div>
			</div>
			<!--内容-->
			<div class="records-content" :class="inWechat ? 'records-content-inwechat' : 'records-content-notwechat'" v-if="recordList.length>0">
				<!--记录列表-->
				<vu-loadmore :auto-fill=false :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmoreVipRecords" @bottom-status-change="handleBottomChange" :bottom-pull-text="bottomPullText" :bottom-drop-text="bottomDropText" :bottom-loading-text="bottomLoadingText" class="loadmore-component">
				    <ul class="records-list">
						<li v-for="item in recordList" class="record-item">
							...
							<div class="record-money">
								<span>+{{(Number(item.inviteRebate)/100).toFixed(2)}}元</span>
							</div>
						</li>
					</ul>
				  </vu-loadmore>	
			...
			<div class="no-records" v-if="recordList.length == 0">
				...
				<p class="no-content">暂无邀请人员，快去邀请吧~</p>
...
<script type="text/javascript">
	import { getMyInviteList } from '@/api/inviteList'
	...
	export default{
		...
		methods:{
			...
			loadBottom(){
				...
				this.$refs.loadmoreVipRecords.onBottomLoaded();
			},
			/*请求数据*/
			async getRecords(){
				//请求数据
				let res=await getMyInviteList({
					pageNo:this.pageNo+1,
					pageSize:this.pageSize
				})
				if(res.data.code === 0){
					let result=res.data.data.myInviteInfoVOList;
					let pageInfo=res.data.data.pageBean;
					for(let i=0,length=result.length;i<length;i++){
						result[i].time=this.timestampToDate(result[i].inviteTime.toString())
						this.recordList.push(result[i])
					}
